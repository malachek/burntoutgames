...
const NAV = [
  { label: 'Home', href: '/' },
  { label: 'Games', href: '/games/', items: [
    { label: 'EXO', href: '/games/exo/' },
    { label: "Kawai'ian Isolation", href: '/games/kawaiian-isolation/' },
    { label: 'Burning Out', href: '/games/burning-out/' },
  ] },
  { label: 'Team', href: '/team/', items: [
    { label: 'Members', href: '/team/#team' },
    { label: 'About', href: '/team/#studio' },
  ] },
...
export function layout({ site, title, description, path = '/', image = '', jsonLd = null, noindex = false, preload = '', content }) {
  const fullTitle = path === '/' ? `${site.studioName} — ${site.tagline}` : `${title} · ${site.studioName}`;
  const raw = (description || site.descriptor).replace(/\s+/g, ' ').trim();
  const desc = raw.length <= 158 ? raw : raw.slice(0, 155).replace(/[\s,;:—-]+\S*$/, '') + '…';
  const base = site.url.replace(/\/$/, '');
  const ogImage = base + (image || '/assets/og-default.png');

  // Top level links still go somewhere on click; the menu is an extra way in,
  // opened on hover and on keyboard focus so it is not mouse-only.
  const navHtml = NAV.map((n) => {
    const here = path === n.href || (n.href !== '/' && path.startsWith(n.href));
    const link = `<a href="${n.href}"${here ? ' aria-current="page"' : ''}>${esc(n.label)}</a>`;
    if (!n.items) return `<div class="nav__item">${link}</div>`;
    return `<div class="nav__item nav__item--has-menu">
            ${link}
            <div class="nav__menu">
              ${n.items.map((i) => `<a href="${esc(i.href)}">${esc(i.label)}</a>`).join('\n              ')}
            </div>
          </div>`;
  }).join('\n          ');
...
<meta property="og:image:alt" content="${esc(site.studioName)}">
...
${preload ? `<link rel="preload" href="${esc(preload)}" as="image" fetchpriority="high">\n` : ''}
    ...
    <div class="site-footer__grid">
      ...
      <div>
        ...
        <ul>
          ...
          <li><a href="/team/#team">The team</a></li>
...
`;
  const website = {
    ...
    name: site.studioName, url: site.url, publisher: { '@id': site.url + '#org' },
  };
  return layout({
    site, title: 'Home', description: site.descriptor, path: '/',
    jsonLd: [jsonLd, website], preload: '/assets/hero-strip.jpg', content,
  });
}

export function gamesIndexPage({ site, games }) {
  const content = `
<section class="section section--flush-top on-ink">
  <div class="wrap">
    <span class="eyebrow">Burnt Out Games</span>
    <h1 class="games-title">Our games</h1>
    <div class="game-rows">
      ${games.map((g) => gameCard(g, 2)).join('\n      ')}
    </div>
  </div>
</section>
...
export function gamePage({ site, game }) {
  const base = site.url.replace(/\/$/, '');
  const steamUrl = (game.links || []).map((l) => l.url).find((u) => /store\.steampowered\.com/.test(u || ''))
    || (/store\.steampowered\.com/.test(game.primaryCta?.url || '') ? game.primaryCta.url : '');
  // "Free" and "$4.99" live in the fact list; turn them into a real Offer so a
  // search result can show the price instead of guessing.
  const relFact = (game.facts || []).find((f) => /^released$/i.test(f.label || ''));
  const relDate = relFact ? new Date(relFact.value) : null;
  const released = relDate && !Number.isNaN(relDate.valueOf()) ? relDate.toISOString().slice(0, 10) : '';
  const priceFact = (game.facts || []).find((f) => /price/i.test(f.label || ''));
  const priceRaw = priceFact ? String(priceFact.value) : '';
  const price = /free/i.test(priceRaw)
    ? { amount: '0', availability: 'https://schema.org/InStock' }
    : (priceRaw.match(/([0-9]+(?:\.[0-9]{2})?)/)
        ? { amount: priceRaw.match(/([0-9]+(?:\.[0-9]{2})?)/)[1], availability: 'https://schema.org/InStock' }
        : null);

  // Breadcrumbs earn the "Burnt Out Games › Games › EXO" trail in results.
  const breadcrumbs = {
    '@context': 'https://schema.org', '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: base + '/' },
      { '@type': 'ListItem', position: 2, name: 'Games', item: base + '/games/' },
      { '@type': 'ListItem', position: 3, name: game.title, item: `${base}/games/${game.slug}/` },
    ],
  };
  ...
  const jsonLd = {
    ...
    ...(game.cover ? { image: base + game.cover } : {}),
    ...(released ? { datePublished: released } : {}),
    ...(steamUrl ? { sameAs: [steamUrl] } : {}),
    ...(price ? { offers: { '@type': 'Offer', price: price.amount, priceCurrency: 'USD', availability: price.availability, url: steamUrl || base + `/games/${game.slug}/` } } : {}),
...
`;
  return layout({
    // The genre is the phrase people search; "EXO" alone competes with the world.
    site, title: `${game.title} — ${game.genre}`,
    description: `${game.tagline} ${game.cardBlurb}`,
    path: `/games/${game.slug}/`, image: game.cover || '',
    jsonLd: [jsonLd, breadcrumbs], content,
  });
}
  ...
  <div class="wrap">
    ...
    <ul class="team-grid">
      ${team.map((p) => {
        ...
        // A dev can have both. `website`/`url` are the older field names.
        const portfolio = p.portfolio || p.website || p.url || '';
        ...
        <div class="person__body">
          ...
          <p class="person__links">${[
            portfolio ? `<a href="${esc(portfolio)}" target="_blank" rel="noopener noreferrer" aria-label="${esc(p.name)} — portfolio">${GLOBE_MARK}</a>` : '',
...
<section class="section on-ink" id="studio">
  <div class="wrap studio-split">
    <div>
      <span class="eyebrow">The studio</span>
      <h1>${esc(site.aboutHeading || 'We game')}</h1>
      <div class="prose lead" style="margin-top:var(--s-5)">${renderMarkdown(site.aboutLong)}</div>
    </div>
    <figure class="studio-photo">
      <img src="/assets/team/team-photo.jpg" alt="The Burnt Out Games team and EXO contributors at a Riot Games event" width="1280" height="853" loading="lazy">
      <figcaption>${esc(site.teamPhotoCaption || '')}</figcaption>
    </figure>
  </div>
</section>
...
<section class="section on-peach">
  <div class="wrap">
    ...
    <ul class="shipped">
      ${games.map((g) => `<li class="shipped__item">
        <a class="shipped__link" href="/games/${esc(g.slug)}/" style="${themeVars(g)}">
          <span class="shipped__art">
            ${g.cover ? `<img src="${esc(g.cover)}" alt="" aria-hidden="true" loading="lazy" width="1440" height="480">` : ''}
            ${g.mark ? `<img class="shipped__mark" src="${esc(g.mark)}" alt="" aria-hidden="true" loading="lazy">` : ''}
          </span>
          <span class="shipped__text">
            <span class="shipped__title">${esc(g.title)}</span>
            <span class="shipped__genre">${esc(g.genre)}</span>
            <span class="shipped__status">${esc(g.status)}${g.price ? ` · ${esc(g.price)}` : ''}</span>
          </span>
        </a>
      </li>`).join('\n      ')}
    </ul>
...
`;
  return layout({
    ...
    path: '/team/', image: '/assets/team/team-photo.jpg', content,
  });
}
...
<section class="section on-peach">
  <div class="wrap">
    ...
    <div class="grid grid--3">
      ${posts.map((p) => `<article class="card post-card reveal">
        ...
        <div class="card__body">
          ...
          <h3 class="card__title"><a class="post-card__link" href="/devlog/${esc(p.slug)}/">${esc(p.title)}</a></h3>
          <p>${esc(p.summary || toPlainText(p.body, 160))}</p>
          <div class="card__foot"><span class="post-card__cue">Read it</span></div>
        </div>
      </article>`).join('\n      ')}
    </div>`
...
<article class="section on-peach">
  <div class="wrap">
    ...
    <p style="margin-top:var(--s-8)"><a class="btn btn--ghost" href="/devlog/">All devlog posts</a></p>
  </div>
</article>
...
`;
  const crumbs = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Devlog', item: site.url.replace(/\/$/, '') + '/devlog/' },
      { '@type': 'ListItem', position: 3, name: post.title, item: `${site.url.replace(/\/$/, '')}/devlog/${post.slug}/` },
    ],
  };
  return layout({ site, title: post.title, description: post.summary || toPlainText(post.body, 160), path: `/devlog/${post.slug}/`, image: post.cover || '', jsonLd: [jsonLd, crumbs], content });
}
  ...
  <div class="wrap">
    <div class="grid grid--2" style="gap:var(--s-8);align-items:start">
      <div>
        ...
        <p class="lead" style="margin-top:var(--s-3);max-width:46ch">${esc(site.contactBlurb || '')}</p>
        <form class="form" id="contact-form" method="POST" action="/api/contact" style="margin-top:var(--s-5)">
          ...
          <p class="form__note">Goes straight to <a href="mailto:${esc(site.email)}">${esc(site.email)}</a>. We read all of it.</p>
        </form>
      </div>
...
`;
  return layout({
    site, title: 'Contact',
    description: `Talk to ${site.studioName} about press, publishing, playtests or feedback. Studio fact sheet and boilerplate for anyone writing about EXO, Kawai'ian Isolation or Burning Out.`,
...
<section class="hero" style="text-align:center">
  ...
  return layout({ site, title: 'Page not found', description: `That page has moved or never existed. Everything ${site.studioName} makes is one link away.`, path: '/404.html', noindex: true, content });
}
